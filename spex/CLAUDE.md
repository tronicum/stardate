...
spex frame-sequence <in1> <in2> ... -o <dir>  # tile N point clouds sharing one offset into a playable animation
spex serve <tileset-dir>                  # serve + open browser (works for both a single tileset and a frame-sequence dir)
```
...
spex deb-deps <package> -o <graph.json>            # real `dpkg -s`: one level of direct deps, Debian/Ubuntu only
spex cargo-deps <package> -o <graph.json>          # real `cargo tree -p`: run from inside a Cargo project
spex npm-deps -o <graph.json>                      # real `npm ls --json --all`: run from inside an npm project
...
spex ascii <tileset-dir> --animate [--frames --fps --loops]  # same, but a turntable-orbit animation played live in the terminal
spex ascii <tileset-dir> --out <file.html> [--frames --fps]  # same animation as a self-contained animated HTML file
```
...
- **spex-tiler** — builds the octree tileset from a flat `Vec<Point>`, used identically by both pipelines: recursively partitions by bounding-box octant, reservoir-samples a fixed point budget per node for that LOD level, writes non-overlapping `octree/<node-id>.bin` files + a `tileset.json` manifest. `build()` returns the coordinate offset it subtracted — callers with other data in the same original coordinate space (e.g. spex-graph's node centers) need it to stay aligned with the tileset's points. `build_with_offset()` is the same, but lets a caller force a specific offset instead of always deriving one from that call's own bounding box — needed by `spex frame-sequence` so several independently-tiled animation frames share one coordinate frame (`build()` is a thin wrapper calling this with `None`). `read_points()` is the read-side counterpart — reads any tileset back into a flat `Vec<Point>` (used by `spex ascii`).
...
- **spex-cli** — the `spex` binary. Each adapter is its own module (`trace.rs`, `ps_tree.rs`, `brew_deps.rs`, `deb_deps.rs`, `cargo_deps.rs`, `disk_usage.rs`, `sql_schema.rs`, `pstree_demo.rs`) that shells out to a real tool, parses its text output, and returns a `spex_graph::Graph`. `deb_deps.rs` shells out to `dpkg -s` (Debian/Ubuntu only) — one level of real direct dependencies (`Depends:` field), real installed size (KB) driving color; only the first `|`-separated alternative of a dependency spec is kept (same "pick one, `Graph` is tree-only" tradeoff as `sql-schema`'s first-FK rule). `cargo_deps.rs` shells out to `cargo tree -p <package>` (run from inside a real Cargo project — `scripts/walkthrough.sh` dogfoods it on `spex-graph` itself) — same box-drawing-tree parsing technique as `brew_deps.rs` (4-char prefix chunks give each line's depth/parent), adapted for cargo's `name vX.Y.Z (annotation)` line shape and its `[build-dependencies]`/`[dev-dependencies]` section-header lines (skipped, not packages). `npm_deps.rs` shells out to `npm ls --json --all` (run from inside a real npm project — dogfooded on `viewer/`'s own `package-lock.json`) — unlike `brew`/`cargo`, `npm ls` has a real `--json` mode, so this parses actual structured JSON (`{"dependencies": {"pkg-name": {"version", "dependencies": {...}}}}`) instead of scraping tree art; parses regardless of `npm ls`'s exit code, since it commonly exits non-zero on real-world peer-dependency warnings even with perfectly valid JSON output. `sql_schema.rs` shells out to `sqlite3` (schema/FK/row-count introspection via `PRAGMA` queries) — a table's first foreign key becomes its tree parent, tables with none are forest roots (same shared-parent limitation as `brew-deps`, see Known limitations). `nav.rs` is the `spex nav` TUI (ratatui/crossterm): list + detail view over `discover_demos()`, a `/`-filter on the list (`filtered_indices()` — `selected_demo()`/navigation/rendering all resolve through it, so the filter narrows what's shown without desyncing which demo is selected), `w` spawns a real `spex serve` as a detached subprocess (own binary, `std::env::current_exe()`) on a deterministic per-demo port rather than running the server in-process. The detail view renders real ANSI truecolor (via the `ansi-to-tui` crate parsing `format_tree()`'s own color-gated output directly into ratatui `Style`s — no special-casing needed, since `format_tree()`'s `stdout().is_terminal()` check is already true inside a running TUI). `ascii.rs` is `spex ascii`: a simple pinhole camera (matches the viewer's default framing) projects `spex_tiler::read_points()`'s output into a z-buffered character grid, luminance→glyph ramp + ANSI truecolor per cell (same technique as `spex_graph::display`). `--animate` reprojects from `orbit_camera()` (a parameterized version of the same default camera, now walked around a horizontal ring) at N evenly-spaced angles, sharing one crop window across all frames so the visible window doesn't jump between angles, and either plays them live in the terminal (clear+redraw) or (`--out <file.html>`) writes them as a self-contained animated HTML page (frames embedded as a JSON array, cycled by an inline `setInterval` — no canvas/WebGL/new dependency). `export_static.rs` is `spex export-static`: writes every demo's tileset + a self-contained per-demo copy of the viewer under `<out>/d/<name>/`, plus a gallery `index.html` at the root, via `spex_server::write_viewer_assets`/`render_gallery_html` — no server involved, for hosting on a static host like GitHub Pages (see `.github/workflows/pages.yml`). `frame_sequence.rs` is `spex frame-sequence`: tiles N real point-cloud files (each one frame of a real animation, e.g. `spex brick-assembly`'s parts-converging-into-a-monolith frames) into N tilesets under `frame-NNN/`, computing one shared bounding box across every frame first so `spex_tiler::build_with_offset` can force the same coordinate offset on all of them — without that, each frame's independently-derived offset would make the point cloud's position jump every time the viewer switched frames. Writes a `sequence.json` manifest (`{version, frameCount, fps, frames}`) the viewer reads to drive playback; `cmd_serve` accepts either a plain tileset directory or one with `sequence.json` at its root. `main.rs` wires clap subcommands to all of these plus the point-cloud/tileset/serve commands. `tests/schema_validation.rs` and `tests/end_to_end_journey.rs` are black-box integration tests (spawn the built binary — spex-cli has no lib target) — the latter's `run_full_pipeline()` helper is a reusable template for exercising any adapter/pipeline end to end.
- **viewer/** — Vite + TypeScript + three.js SPA (not a Cargo crate). Same bundle serves both server modes: `TILESET_BASE` is resolved from `location.pathname` (`/d/<name>/` → `/d/<name>/tileset`, otherwise plain `/tileset`). Fetches `tileset.json`, streams octree nodes via a priority-queue LOD selector (screen-space-error-ordered, frustum-culled, point-budgeted — mirrors Potree/3D-Tiles refinement), and optionally fetches `nodes.json` (hover tooltips) and `meta.json` (persistent title + color-gradient legend). `packetAnimation.ts`'s `buildPrimaryPath()` (always-first-child walk from the root) drives a marker mesh animated node-to-node along a graph's primary chain, toggleable via the "Animate packet" control. Also optionally fetches `sequence.json` (`fetchSequence` in `tileset.ts`) — if present (a `spex frame-sequence` output), `main.ts` points a mutable `activeBase` at whichever frame is current, computes one combined camera framing across every frame's bounds up front (`mergeBounds`) so the camera doesn't reframe on each swap, and on a `sequence.fps`-driven timer unloads the current frame's octree nodes and lets the very same LOD selector/`ensureLoaded` path load the next frame's — a real point-cloud animation with zero new rendering code.
...
- `sequence.json` (`spex frame-sequence` output only) — `{version, frameCount, fps, frames}`, sitting alongside `frame-000/`, `frame-001/`, ... subdirectories (each an ordinary tileset, sharing one coordinate offset). The viewer plays these back as a real point-cloud animation instead of loading one static tileset.

Formal JSON Schemas for all five (`graph.json` too) live in `spec/*.schema.json` — see `spec/README.md`.
...
- `Graph`/`layout::place()` are tree-only for *position* — a node still gets exactly one 3D position, via its single `parent`. `brew-deps` (only, as of issue #24) merges a genuinely shared package (reachable from more than one branch) into one real `GraphNode` instead of duplicating it, recording every other real parent in `GraphNode.extra_parents`/`nodes.json`'s `extraParents` (rendered as extra viewer edge lines) rather than a second position. `npm-deps`/`cargo-deps` have the identical duplication pattern and are an explicit follow-up, not yet fixed. `sql-schema` still only draws a table's *first* foreign key as its tree parent (additional FKs are recorded in metadata, not as a second edge at all — a narrower case than `extra_parents` since a table can have several real FKs, not just one "other real parent").
