...
### Doing / next up
- [ ] Historical stock-price demo for fun (Volkswagen, Tesla, BYD) — daily/weekly closes as a chain, metric = price or volume, colored by gain/loss. Start with one ticker as a single chain (real exercise for the animated-packet chain view); later, one packet per ticker running in parallel on separate chains side by side — a nice concrete forcing case for the "multiple simultaneous packets" work below, since three unrelated single-root chains is simpler than one branching tree
