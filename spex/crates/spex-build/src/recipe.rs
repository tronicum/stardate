...
pub fn build_recipe(recipe: &Recipe) -> Result<Vec<Placement>> {
    ...
    for step in &recipe.steps {
        ...
        for (origin, orientation) in origins {
            ...
            all.extend(prim.emit(origin, orientation));
        }
    }
    Ok(all)
}
...
mod tests {
    ...
    fn parses_a_real_recipe_and_builds_real_placements() {
        ...
        assert_eq!(placements.len(), 4, "8 studs, stack bond -> 4 real 1x2 stretchers");
    ...
    fn write_ldr_carries_real_provenance() {
        ...
        assert_eq!(text.lines().filter(|l| l.starts_with("1 ")).count(), 4);
    }
