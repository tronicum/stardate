...
/// The graph packet marker(s) (see `packet.rs`): letters, not ramp glyphs, so
/// they read as "a distinct thing" rather than "a brighter point" — RAMP is
/// all punctuation, so any letter is unambiguous — colored bright, saturated
/// hues the metric-driven blue->yellow->red blob gradient never produces
/// (`spex_graph::layout`), so a marker never blends into real graph data.
/// issue #26: one marker per the root's direct children now animate
/// concurrently (see `packet::concurrent_packet_states`), so both glyph and
/// color are indexed by marker, cycling if there are ever more concurrent
/// markers than palette entries (bounded in practice by the layout's own
/// `MAX_CHILDREN_SHOWN` fan-out cap of 20). Index 0 keeps the original
/// single-marker 'P'/magenta pairing, so a chain's one marker (the common
/// case — a single top-level process, traceroute hops) renders exactly as
/// it did before #26.
const PACKET_MARKER_GLYPHS: &[char] = &['P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y'];
const PACKET_MARKER_COLORS: &[[u8; 3]] = &[
    [255, 0, 255],   // magenta (the original, pre-#26 single-marker color)
    [0, 255, 255],   // cyan
...
fn packet_marker_glyph(marker_index: usize) -> char {
    PACKET_MARKER_GLYPHS[marker_index % PACKET_MARKER_GLYPHS.len()]
}

fn packet_marker_color(marker_index: usize) -> [u8; 3] {
    PACKET_MARKER_COLORS[marker_index % PACKET_MARKER_COLORS.len()]
}
...
/// produce byte-identical formatting for the same cells. `markers`, if
/// non-empty, is that frame's per-marker `(row, col)` cell in full
/// (uncropped) grid coordinates (index = marker id, `None` if that marker
/// isn't in view this frame) — whichever marker's cell matches gets drawn
/// overwriting whatever point (if any) landed there, on top of the regular
/// points, per the graph packet overlay (see `packet_overlay`). An empty
/// slice (the plain point-cloud/no-graph-data case) costs nothing extra per
/// cell — the lookup short-circuits immediately.
fn grid_slice_to_ansi(grid: &[Option<[u8; 3]>], width: usize, min_row: usize, max_row: usize, min_col: usize, max_col: usize, use_color: bool, markers: &[Option<(usize, usize)>]) -> String {
    let mut out = String::new();
    for row in min_row..=max_row {
        for col in min_col..=max_col {
            if let Some(marker_index) = markers.iter().position(|m| *m == Some((row, col))) {
                out.push_str(&packet_marker_ansi(use_color, marker_index));
                continue;
            }
...
fn packet_marker_ansi(use_color: bool, marker_index: usize) -> String {
    let glyph = packet_marker_glyph(marker_index);
    if use_color {
        let color = packet_marker_color(marker_index);
        format!("\x1b[38;2;{};{};{}m{glyph}\x1b[0m", color[0], color[1], color[2])
    } else {
        glyph.to_string()
    }
}
...
/// Per-frame graph packet overlay: where to draw each concurrent marker (if
/// it projects inside that frame's view at all — see issue #26) and the
/// hop-readout text to print beneath the grid — always present as long as
/// `nodes` yields at least one real marker, even on a frame where every
/// marker itself falls outside the crop, so "what hop is each packet on"
/// stays answerable every frame. `frame_count` must match the number of
/// grids the caller projected, so markers/footer line up 1:1 with frames.
struct PacketOverlay {
    /// One entry per frame, itself one entry per concurrent marker (index =
    /// marker id, stable across frames): `(row, col)` in full (uncropped)
    /// grid coordinates, or `None` if that marker isn't in view that frame.
    markers: Vec<Vec<Option<(usize, usize)>>>,
    /// One footer per frame — a single "hop N/M: from -> to" line for a lone
    /// marker (byte-identical to the pre-#26 single-packet footer), or one
    /// glyph-prefixed line per marker (newline-joined) when there are
    /// several concurrent markers to report on.
    footers: Vec<String>,
}

/// Builds the overlay from a tileset's whole `nodes.json` (not a pre-swept
/// path — issue #26's `concurrent_packet_states` needs the full node list to
/// derive one path per root child). Returns `None` if there's nothing real
/// to animate (fewer than two nodes, a childless root, or a graph with no
/// root at all).
fn packet_overlay(bounds: &Aabb, width: usize, height: usize, nodes: &[NodeLabel], frame_count: usize) -> Option<PacketOverlay> {
    let all_states = packet::concurrent_packet_states(nodes, frame_count);
    if all_states.is_empty() {
        return None;
    }
    let marker_count = all_states.len();
    let mut markers = Vec::with_capacity(frame_count);
    let mut footers = Vec::with_capacity(frame_count);
    for frame in 0..frame_count {
        let camera = orbit_camera(bounds, frame_angle(frame, frame_count));
        let mut frame_markers = Vec::with_capacity(marker_count);
        let mut lines = Vec::with_capacity(marker_count);
        for (m, states) in all_states.iter().enumerate() {
            let state = &states[frame];
            frame_markers.push(project_point(state.position, &camera, width, height));
            lines.push(if marker_count == 1 {
                format!("hop {}/{}: {} -> {}", state.hop_index, state.hop_count, state.from_label, state.to_label)
            } else {
                format!("{}: hop {}/{}: {} -> {}", packet_marker_glyph(m), state.hop_index, state.hop_count, state.from_label, state.to_label)
            });
        }
        markers.push(frame_markers);
        footers.push(lines.join("\n"));
    }
    Some(PacketOverlay { markers, footers })
}

/// One ANSI-colored string per frame of a turntable orbit animation — same
/// crop window for every frame (see [`project_frames_cropped`]). `nodes`, if
/// given a real tileset's `nodes.json` content (see `packet.rs`), overlays
/// one distinct marker glyph per concurrently-sweeping root child (issue
/// #26) plus a trailing per-marker "hop N/M: ..." readout on every frame;
/// `None` reproduces the exact pre-graph-awareness output (plain
/// point-cloud tilesets have no `nodes.json` to build markers from).
pub fn render_frames(points: &[Point], width: usize, frame_count: usize, nodes: Option<&[NodeLabel]>) -> Vec<String> {
    let Some((grids, bounds, width, height, min_row, max_row, min_col, max_col)) = project_frames_cropped(points, width, frame_count) else {
        return Vec::new();
    };
    let use_color = std::io::stdout().is_terminal() && std::env::var_os("NO_COLOR").is_none();
    let overlay = nodes.and_then(|n| packet_overlay(&bounds, width, height, n, grids.len()));
    let no_markers: Vec<Option<(usize, usize)>> = Vec::new();
    grids
        .iter()
        .enumerate()
        .map(|(i, g)| {
            let markers = overlay.as_ref().map(|o| &o.markers[i]).unwrap_or(&no_markers);
            let mut frame = grid_slice_to_ansi(g, width, min_row, max_row, min_col, max_col, use_color, markers);
...
pub fn run_animated(tileset_dir: &Path, width: usize, frame_count: usize, fps: f64, loops: usize) -> Result<()> {
    let points = spex_tiler::read_points(tileset_dir)?;
    let nodes = packet::load_nodes(tileset_dir)?;
    let frames = render_frames(&points, width, frame_count, nodes.as_deref());
    if frames.is_empty() {
        return Ok(());
    }
...
/// `render_html` and the multi-frame animated HTML export. `markers` is the
/// same per-marker "draw this cell as a packet regardless of what's under
/// it" list as `grid_slice_to_ansi`'s (index = marker id).
fn grid_slice_to_html_body(grid: &[Option<[u8; 3]>], width: usize, min_row: usize, max_row: usize, min_col: usize, max_col: usize, markers: &[Option<(usize, usize)>]) -> String {
        ...
        for col in min_col..=max_col {
            if let Some(marker_index) = markers.iter().position(|m| *m == Some((row, col))) {
                ...
                body.push_str(&format!(
                    ...
                    color[1],
                    color[2],
                    html_escape_char(packet_marker_glyph(marker_index))
                ));
                continue;
            }
            match grid[row * width + col] {
...
/// `run_animated` is: a real `nodes.json` next to `tileset_dir` gets one
/// packet marker per concurrently-sweeping root child (issue #26) plus a
/// per-marker hop-readout line baked into every frame's body, so the same
/// single `innerHTML` swap updates the grid and the readout together.
pub fn run_html_animated(tileset_dir: &Path, width: usize, frame_count: usize, fps: f64, title: &str) -> Result<String> {
    let points = spex_tiler::read_points(tileset_dir)?;
    let nodes = packet::load_nodes(tileset_dir)?;
    Ok(render_html_animated(&points, width, frame_count, fps, title, nodes.as_deref()))
}

fn render_html_animated(points: &[Point], width: usize, frame_count: usize, fps: f64, title: &str, nodes: Option<&[NodeLabel]>) -> String {
    let Some((grids, bounds, width, height, min_row, max_row, min_col, max_col)) = project_frames_cropped(points, width, frame_count) else {
        return format!("<!doctype html><title>{title}</title><body style=\"background:#0b0e12\"></body>");
    };
    let overlay = nodes.and_then(|n| packet_overlay(&bounds, width, height, n, grids.len()));
    ...
    let bodies: Vec<String> = grids
        ...
        .map(|(i, g)| {
            let markers = overlay.as_ref().map(|o| &o.markers[i]).unwrap_or(&no_markers);
            let mut body = grid_slice_to_html_body(g, width, min_row, max_row, min_col, max_col, markers);
            if let Some(o) = &overlay {
                body.push('\n');
...
mod tests {
    ...
    /// the tests above, so `nodes: Some(..)` output can be compared directly
    /// against `nodes: None` output on identical underlying point data. Its
    /// root has exactly one child, so this exercises the degenerate
    /// single-marker case (see `packet.rs`'s `build_concurrent_sweep_paths`
    /// doc comment) — `sample_branching_nodes` below covers real concurrent
    /// markers.
    fn sample_nodes() -> Vec<NodeLabel> {
        vec![
    ...
    /// A real branching graph: a root with two children, e.g. a process
    /// forking two children at once — the concurrent-marker case issue #26
    /// introduced. `heavy` outweighs `light` so it sweeps first/gets marker
    /// index 0, same ordering convention as `packet.rs`'s own tests.
    fn sample_branching_nodes() -> Vec<NodeLabel> {
        vec![
            NodeLabel { id: "root".into(), label: "root".into(), parent: None, center: [0.0, 0.0, 0.0], metric: Some(0.0), metadata: Default::default() },
            NodeLabel { id: "heavy".into(), label: "heavy".into(), parent: Some("root".into()), center: [3.0, 0.0, 0.0], metric: Some(5.0), metadata: Default::default() },
            NodeLabel { id: "light".into(), label: "light".into(), parent: Some("root".into()), center: [0.0, 3.0, 0.0], metric: Some(1.0), metadata: Default::default() },
        ]
    }
    ...
    fn no_nodes_json_reproduces_the_pre_graph_aware_output_exactly() {
        ...
        // this feature exists — `nodes: None` is exactly what `run_animated`
        // passes when `nodes.json` is absent.
        let points = vec![Point { position: [3.0, 0.0, 0.0], color: [255, 255, 255] }, Point { position: [-3.0, 0.0, 0.0], color: [80, 80, 200] }];
        let without_nodes = render_frames(&points, 60, 8, None);
        let also_without_nodes = render_frames(&points, 60, 8, None);
        assert_eq!(without_nodes, also_without_nodes);
        assert!(without_nodes.iter().all(|f| !f.contains("hop ")), "no footer line should appear without graph data");
    }

    #[test]
    fn a_single_child_root_adds_a_hop_readout_footer_and_a_marker_glyph() {
        let points = vec![Point { position: [3.0, 0.0, 0.0], color: [255, 255, 255] }, Point { position: [-3.0, 0.0, 0.0], color: [80, 80, 200] }];
        let nodes = sample_nodes();
        let frames = render_frames(&points, 60, 8, Some(&nodes));
        assert_eq!(frames.len(), 8);
        assert!(frames.iter().all(|f| f.contains("hop ") && f.contains('/')), "every frame should carry a hop readout line");
        // A single-marker (one root child) footer is byte-identical to the
        // pre-#26 single-packet phrasing — no "P: " glyph prefix needed
        // when there's nothing else to disambiguate from.
        assert_eq!(frames[0].lines().last().unwrap(), "hop 1/1: root -> child", "frame 0 should read hop 1/1, got: {}", frames[0]);
        ...
        assert!(frames.iter().any(|f| f.contains(packet_marker_glyph(0))), "expected the packet marker glyph to show up in at least one frame");
    }

    #[test]
    fn a_branching_root_animates_multiple_concurrent_markers() {
        // issue #26: a root with two children should produce two distinct
        // markers, each with its own hop readout, on the same frames.
        let points = vec![
            Point { position: [0.0, 0.0, 0.0], color: [255, 255, 255] },
            Point { position: [3.0, 0.0, 0.0], color: [80, 80, 200] },
            Point { position: [0.0, 3.0, 0.0], color: [200, 80, 80] },
        ];
        let nodes = sample_branching_nodes();
        let frames = render_frames(&points, 80, 16, Some(&nodes));
        assert_eq!(frames.len(), 16);
        // Every frame's footer should report on both markers — one line
        // each, glyph-prefixed since there's more than one.
        for f in &frames {
            let footer_lines: Vec<&str> = f.lines().filter(|l| l.contains("hop ")).collect();
            assert_eq!(footer_lines.len(), 2, "expected one hop line per concurrent marker, got: {footer_lines:?} in frame:\n{f}");
            assert!(footer_lines[0].starts_with(&format!("{}: hop", packet_marker_glyph(0))));
            assert!(footer_lines[1].starts_with(&format!("{}: hop", packet_marker_glyph(1))));
        }
        // Both distinct marker glyphs should show up somewhere across the
        // animation (real, distinguishable markers, not one glyph reused).
        assert!(frames.iter().any(|f| f.contains(packet_marker_glyph(0))), "marker 0's glyph should appear in some frame");
        assert!(frames.iter().any(|f| f.contains(packet_marker_glyph(1))), "marker 1's glyph should appear in some frame");
    }
    ...
    fn html_export_embeds_the_hop_readout_alongside_each_frame() {
        let points = vec![Point { position: [3.0, 0.0, 0.0], color: [255, 255, 255] }, Point { position: [-3.0, 0.0, 0.0], color: [80, 80, 200] }];
        let nodes = sample_nodes();
        let html = render_html_animated(&points, 60, 6, 10.0, "test", Some(&nodes));
        let json_start = html.find("const frames = ").unwrap() + "const frames = ".len();
        let json_end = html[json_start..].find(";\n").unwrap() + json_start;
        let parsed: Vec<String> = serde_json::from_str(&html[json_start..json_end]).unwrap();
        assert_eq!(parsed.len(), 6);
